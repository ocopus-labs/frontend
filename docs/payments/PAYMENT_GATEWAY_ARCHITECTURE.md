...
export interface IPaymentGateway {
	 ...
	 */
	handleWebhook(payload: Buffer, signature: string): Promise<WebhookProcessingResult | null>;
...
export class StripeAdapter implements IPaymentGateway {
	...
	async handleWebhook(payload: Buffer, signature: string): Promise<WebhookProcessingResult | null> {
		try {
...
export class PaymentService {
	...
	async startPosPayment(
		...
		return gateway.createPosPayment(amount, currency, terminalId, orderId, {
			businessId,
			storeId,
...
@Module({
	imports: [HttpModule, BusinessModule, OrderModule],
	controllers: [PaymentController, PaymentWebhookController],
	providers: [PaymentService, StripeAdapter, PineLabsAdapter, DodoPaymentsAdapter],
...
export class PaymentWebhookController {
	...
	async handleStripe(
		...
		const result = await this.stripeAdapter.handleWebhook(req.rawBody, signature);
	...
	async handlePineLabs(
		...
		const result = await this.pineLabsAdapter.handleWebhook(req.rawBody, signature);
	...
	async handleDodoPayments(
		...
		const result = await this.dodoPaymentsAdapter.handleWebhook(req.rawBody, signature);
