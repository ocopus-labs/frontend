...
export const load: PageServerLoad = async ({ params, locals }) => {
  try {
    ...
    const menu = await fetch(
      `${process.env.API_URL}/businesses/${businessId}/menu`,
      {
        headers: {
  ...
  <Dialog.Content class="max-w-2xl">
    ...
    <div class="space-y-6 py-4">
      ...
      {#if item.modifiers?.spiceLevels}
        <div>
          <h3 class="font-medium mb-3">Spice Level</h3>
          <RadioGroup>
      ...
      {#if item.modifiers?.addOns}
        <div>
          <h3 class="font-medium mb-3">Add-ons</h3>
...
<div class="container mx-auto max-w-2xl p-8">
  ...
  {:else if paymentStatus === 'success'}
    <div class="text-center p-8 bg-green-50 border border-green-200 rounded-lg">
      <p class="text-2xl text-green-600 mb-4">✓ Payment Successful!</p>
      <Button onclick={() => goto(`/${business.current!.id}/pos`)}>
        New Order
      </Button>
    </div>
  {:else if paymentStatus === 'failed'}
    <div class="text-center p-8 bg-red-50 border border-red-200 rounded-lg">
      <p class="text-2xl text-red-600 mb-4">✗ Payment Failed</p>
      <Button onclick={() => paymentStatus = 'idle'}>
        Try Again
      </Button>
    </div>
  {:else}
