<script lang="ts">
	...
	const tableNumber = $derived(data.tableNumber);
	...
	interface CartItem {
		...
		unitPrice: number; // basePrice + modifier surcharges
	}
	...
	$effect(() => {
		if (typeof window !== 'undefined') {
			const key = `cart:${business.slug}:${tableNumber}`;
	...
	// Save business info for checkout page
	$effect(() => {
		if (typeof window !== 'undefined' && business?.slug) {
			sessionStorage.setItem(`business:${business.slug}`, JSON.stringify(business));
		}
	});
	...
	function goToCheckout() {
		goto(`/order/${business.slug}/${tableNumber}/checkout`);
	}
		...
		<div class="flex items-center gap-3 px-4 py-3">
			...
			<div class="min-w-0 flex-1">
				...
				<div class="flex items-center gap-1.5 text-xs text-muted-foreground">
					<span
						class="inline-flex items-center gap-1 rounded-md bg-primary/10 px-1.5 py-0.5 font-medium text-primary"
					>
						Table {tableNumber}
					</span>
				</div>
