...
<div class="flex min-h-svh flex-col items-center justify-center bg-gray-50 px-4 py-8 dark:bg-background">
	<div class="w-full max-w-sm space-y-6">
		...
		{#if entry.status === 'seated'}
			<!-- Seated state -->
			<div class="rounded-2xl border border-green-200 bg-green-50 p-8 text-center shadow-sm dark:border-green-900/30 dark:bg-green-900/10">
				<div class="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-green-500 text-white">
					<svg class="h-8 w-8" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
					</svg>
				</div>
				<p class="text-xl font-bold text-green-800 dark:text-green-200">You've Been Seated</p>
				<p class="mt-1 text-sm text-green-600 dark:text-green-400">Enjoy your visit!</p>
			</div>
		{:else if entry.status === 'cancelled' || entry.status === 'no_show'}
			<!-- Removed state -->
			<div class="rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm dark:border-border dark:bg-card">
				<div class="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-gray-200 text-gray-500 dark:bg-muted dark:text-muted-foreground">
				...
				<p class="mt-1 text-sm text-muted-foreground">
					{entry.status === 'cancelled' ? 'This entry has been cancelled.' : 'This entry was marked as a no-show.'}
				</p>
			</div>
		{:else}
			<!-- Active waiting / notified state -->
			{#if isNext}
				<div class="rounded-2xl border-2 border-primary bg-primary/5 p-8 text-center shadow-lg dark:bg-primary/10">
					<div class="mb-2 text-sm font-semibold uppercase tracking-wider text-primary">
						You're Next!
					</div>
					<div class="text-7xl font-black tabular-nums text-primary">
						{entry.position}
					</div>
					<p class="mt-2 text-sm text-muted-foreground">Get ready - your table is almost here</p>
				</div>
			{:else if entry.status === 'notified'}
				<div class="rounded-2xl border-2 border-blue-400 bg-blue-50 p-8 text-center shadow-lg dark:border-blue-600 dark:bg-blue-900/10">
					<div class="mb-3 flex items-center justify-center gap-2">
						<span class="relative flex h-3 w-3">
							<span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-blue-400 opacity-75"></span>
							<span class="relative inline-flex h-3 w-3 rounded-full bg-blue-500"></span>
						</span>
						<span class="text-sm font-semibold uppercase tracking-wider text-blue-600 dark:text-blue-400">Table Ready</span>
					</div>
					<p class="text-lg font-bold text-blue-800 dark:text-blue-200">Please head to the host stand</p>
			...
			{:else}
				<div class="rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm dark:border-border dark:bg-card">
					...
					<div class="text-7xl font-black tabular-nums text-foreground">
						{entry.position}
					</div>
					{#if entry.estimatedWaitMinutes}
						<div class="mt-3 inline-flex items-center gap-1.5 rounded-full bg-muted px-3 py-1.5 text-sm font-medium text-muted-foreground">
		...
		<div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-border dark:bg-card">
			<div class="space-y-3 text-sm">
				<div class="flex justify-between">
					...
					<span class="font-medium">{entry.partySize} {entry.partySize === 1 ? 'guest' : 'guests'}</span>
				</div>
				<div class="flex justify-between">
				...
				<div class="space-y-0">
					{#each statusSteps as step, i}
						...
						<div class="flex gap-3">
							...
							<div class="pb-3">
								<p class="text-sm font-semibold {isCurrent || isCompleted ? 'text-foreground' : 'text-gray-400 dark:text-muted-foreground'}">
									{step.label}
								</p>
