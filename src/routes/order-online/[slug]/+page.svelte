	...
	>
		...
		{#if onlineOrdering.acceptsTakeaway && onlineOrdering.acceptsDelivery}
			<div class="flex gap-2 px-4 py-2">
				<button
					class="flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2 text-sm font-medium transition-all {orderType ===
					'takeaway'
	...
	<div class="flex-1 space-y-5 px-4 py-4">
		...
		{:else if filteredCategories().length === 0 && searchQuery}
			<div class="flex flex-col items-center justify-center py-16 text-center">
				...
				<p class="text-sm text-muted-foreground">No items found for "{searchQuery}"</p>
			</div>
		{/if}
	</div>
				...
				<Drawer.Footer class="border-t">
					...
					{#if onlineOrdering.minOrderAmount > 0 && !meetsMinOrder}
						<p class="mb-2 text-xs text-destructive">
							Minimum order: {formatPrice(onlineOrdering.minOrderAmount)}. Add {formatPrice(
								onlineOrdering.minOrderAmount - cartTotal
							)} more.
						</p>
					{/if}
