...
<div class="flex flex-1 flex-col">
	<div class="@container/main flex flex-1 flex-col gap-4">
		<div class="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
			<PageHeader title="New Purchase Order" description="Create a new purchase order for inventory replenishment">
				{#snippet actions()}
			...
			<div class="grid gap-6 px-6 lg:grid-cols-3">
				<!-- Order Details -->
					...
					<Card.Root>
						...
						<Card.Content class="grid gap-4">
							...
							<div class="grid gap-2">
								...
								<Textarea id="notes" bind:value={notes} placeholder="Add any notes for this order..." rows={3} />
							</div>
						</Card.Content>
					</Card.Root>
											...
											{#if filteredSearchItems.length > 0}
												{#each filteredSearchItems.slice(0, 10) as item (item.id)}
													...
													>
														...
														<span class="text-muted-foreground">{formatCurrency(item.costPerUnit)} / {item.unit}</span>
													</button>
												{/each}
											{:else}
										...
										<div class="rounded-lg border p-3">
											<div class="flex items-center justify-between">
												<div class="min-w-0 flex-1">
													<p class="truncate font-medium text-sm">{item.itemName}</p>
													<p class="text-xs text-muted-foreground">{item.itemSku} | {item.itemUnit}</p>
												</div>
												<Button
													variant="ghost"
													size="icon"
											...
											<div class="mt-2 grid grid-cols-2 gap-2">
												<div class="grid gap-1">
													...
													<Input
														...
														step="0.01"
														value={item.quantity}
														oninput={(e) => updateLineItemQuantity(item.id, parseFloat((e.target as HTMLInputElement).value) || 0)}
													/>
												</div>
												<div class="grid gap-1">
													<label for="price-{item.id}" class="text-xs text-muted-foreground">Unit Price</label>
													<Input
														id="price-{item.id}"
														type="number"
														min="0"
														...
														oninput={(e) => updateLineItemPrice(item.id, parseFloat((e.target as HTMLInputElement).value) || 0)}
													/>
												</div>
											</div>
						...
						<Card.Content>
							<div class="grid gap-3">
								...
								<div class="flex items-center justify-between text-sm">
									...
									<span class="font-medium">{lineItems.reduce((s, i) => s + i.quantity, 0).toFixed(2)}</span>
								</div>
								<hr />
								<div class="flex items-center justify-between">
