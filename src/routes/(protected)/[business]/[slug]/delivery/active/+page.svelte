<script lang="ts">
	...
	const columns: { status: DeliveryStatus; label: string; color: string }[] = [
		...
		{ status: 'assigned', label: 'Assigned', color: 'bg-chart-3/10' },
		{ status: 'picked_up', label: 'Picked Up', color: 'bg-chart-6/10' },
		{ status: 'in_transit', label: 'In Transit', color: 'bg-chart-4/10' }
	];
	...
	async function confirmAssign() {
		...
		try {
			...
			deliveries = deliveries.map((d) => (d.id === assignTargetDelivery!.id ? result.delivery : d));
	...
	async function advanceStatus(delivery: Delivery) {
		...
		try {
			...
			if (nextStatus === 'delivered') {
				...
				deliveries = deliveries.map((d) => (d.id === delivery.id ? result.delivery : d));
...
<PageShell title="Active Deliveries" description="Track and manage ongoing deliveries">
	{#snippet actions()}
...
<Dialog.Root bind:open={assignDialogOpen}>
	<Dialog.Content class="sm:max-w-md">
		<Dialog.Header>
			...
			<Dialog.Description>
				Select a driver for order #{assignTargetDelivery?.order?.orderNumber ||
					assignTargetDelivery?.orderId.slice(0, 8)}
			</Dialog.Description>
		</Dialog.Header>
