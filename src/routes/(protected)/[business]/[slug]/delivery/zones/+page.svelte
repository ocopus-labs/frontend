<script lang="ts">
	...
	import { createZone, updateZone, type DeliveryZone, type CreateZonePayload } from '$lib/api';
	...
	async function addZoneFn() {
		...
		if (!polygon) {
			toast.error(
				'Valid polygon coordinates required (JSON array of [lat, lng] pairs, minimum 3 points)'
			);
			return;
		}
...
<PageShell title="Delivery Zones" description="Manage delivery zones, fees, and estimated times">
	{#snippet actions()}
	...
	<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
		...
		<div class="rounded-lg border bg-card p-4">
			<div class="flex items-center gap-2 text-sm text-muted-foreground">Active Zones</div>
	...
	{:else}
		<EmptyState
			...
			description={zones.length === 0
				? 'Create your first delivery zone to define coverage areas.'
		...
		<div class="grid gap-4 py-4">
			...
			<div class="grid grid-cols-3 gap-4">
				<div class="grid gap-2">
					...
					<Input
						...
						step="0.01"
						min="0"
						bind:value={newZone.deliveryFee}
						placeholder="0.00"
					/>
				</div>
				<div class="grid gap-2">
					...
					<Input
						...
						step="0.01"
						min="0"
						bind:value={newZone.minOrderAmount}
						placeholder="0.00"
					/>
				</div>
				<div class="grid gap-2">
					...
					<Input
						...
						type="number"
						min="1"
						bind:value={newZone.estimatedMinutes}
						placeholder="30"
					/>
				</div>
			</div>
			<div class="grid gap-2">
				...
				<textarea
					...
					placeholder={'[\n  [28.6139, 77.2090],\n  [28.6200, 77.2150],\n  [28.6100, 77.2200]\n]'}
					rows="5"
					class="flex w-full rounded-md border border-input bg-background px-3 py-2 font-mono text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:outline-none"
				></textarea>
				...
				<div class="grid grid-cols-3 gap-4">
					<div class="grid gap-2">
						...
						<Input
							...
							step="0.01"
							min="0"
							bind:value={editingZone.deliveryFee}
						/>
					</div>
					<div class="grid gap-2">
						...
						<Input
							...
							step="0.01"
							min="0"
							bind:value={editingZone.minOrderAmount}
						/>
					</div>
					<div class="grid gap-2">
						...
						<Input
							...
							type="number"
							min="1"
							bind:value={editingZone.estimatedMinutes}
						/>
					</div>
				</div>
				<div class="grid gap-2">
