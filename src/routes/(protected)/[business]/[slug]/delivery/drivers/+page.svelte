...
<div class="flex flex-1 flex-col">
	<div class="@container/main flex flex-1 flex-col gap-4">
		<div class="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
			<PageHeader title="Delivery Drivers" description="Manage your delivery drivers">
				{#snippet actions()}
			...
			<div class="grid grid-cols-2 gap-4 px-6 sm:grid-cols-5">
				...
				<div class="rounded-lg border bg-card p-4">
					...
					<p class="text-2xl font-bold text-amber-600">{stats.busy}</p>
				</div>
				<div class="rounded-lg border bg-card p-4">
							...
							<div class="mt-2 flex items-center justify-between">
								...
								{#if canModify(data.userRole)}
									<div class="flex gap-1">
										<Button variant="ghost" size="sm" class="h-7 text-xs" onclick={() => editDriverFn(driver)}>Edit</Button>
										<Button
											variant="ghost"
											size="sm"
											...
											<Table.Cell class="text-right">
												<div class="flex justify-end gap-1">
													...
													<Button
														variant="ghost"
														size="sm"
														class={driver.isActive ? 'text-destructive hover:text-destructive' : 'text-success hover:text-success'}
														onclick={() => toggleActive(driver)}
													>
														{driver.isActive ? 'Deactivate' : 'Activate'}
													</Button>
			...
			{:else}
				<EmptyState
					...
					description={drivers.length === 0 ? 'Add your first delivery driver to get started.' : 'Try adjusting your search or filters.'}
			...
			<div class="grid gap-4 py-4">
				...
				<div class="grid gap-2">
					...
					<Select.Root type="single" bind:value={editingDriver.vehicleType}>
						<Select.Trigger class="w-full">
							{vehicleTypes.find((v) => v.value === editingDriver?.vehicleType)?.label || 'Select vehicle'}
						</Select.Trigger>
						<Select.Content>
							{#each vehicleTypes as vt}
								<Select.Item value={vt.value}>{vt.label}</Select.Item>
