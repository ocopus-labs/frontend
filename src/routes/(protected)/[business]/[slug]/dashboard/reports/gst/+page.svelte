...
<div class="flex flex-1 flex-col">
	<div class="@container/main flex flex-1 flex-col gap-4">
		<div class="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
			...
			{#if data.error}
				<div class="mx-4 rounded-lg border border-destructive bg-destructive/10 p-4 lg:mx-6">
					<p class="text-sm text-destructive">{data.error}</p>
				</div>
			{:else if summary}
				<!-- Summary Cards -->
				<div class="grid grid-cols-2 gap-4 px-4 sm:grid-cols-3 lg:grid-cols-5 lg:px-6">
					<Card.Root>
					...
					<Card.Root>
						...
						<Card.Content>
							<div class="text-xl font-bold text-blue-600">{formatCurrency(summary.cgstCollected)}</div>
					...
					<Card.Root>
						...
						<Card.Content>
							<div class="text-xl font-bold text-purple-600">{formatCurrency(summary.sgstCollected)}</div>
					...
					<Card.Root class="col-span-2 sm:col-span-1">
						...
						<Card.Content>
							<div class="text-xl font-bold text-amber-600">{formatCurrency(summary.totalTaxCollected)}</div>
								...
								<Table.Root>
									...
									<Table.Body>
										{#each summary.rateSummary as row}
											<Table.Row>
												...
												<Table.Cell class="text-right tabular-nums text-blue-600">
													{formatCurrency(row.cgst)}
												</Table.Cell>
												<Table.Cell class="text-right tabular-nums text-purple-600">
													{formatCurrency(row.sgst)}
												</Table.Cell>
												{#if summary.igstCollected > 0}
													<Table.Cell class="text-right tabular-nums text-orange-600">
														{formatCurrency(row.igst)}
													</Table.Cell>
												{/if}
												<Table.Cell class="text-right tabular-nums font-semibold text-amber-600">
									...
									<Table.Footer>
										<Table.Row>
											...
											<Table.Cell class="text-right font-semibold tabular-nums text-blue-600">
												{formatCurrency(summary.cgstCollected)}
											</Table.Cell>
											<Table.Cell class="text-right font-semibold tabular-nums text-purple-600">
												{formatCurrency(summary.sgstCollected)}
											</Table.Cell>
											{#if summary.igstCollected > 0}
												<Table.Cell class="text-right font-semibold tabular-nums text-orange-600">
													{formatCurrency(summary.igstCollected)}
												</Table.Cell>
											{/if}
											<Table.Cell class="text-right font-semibold tabular-nums text-amber-600">
							...
							<Card.Header>
								<div class="flex items-center justify-between">
									<div>
										...
										<Card.Description>
											Item-level HSN code breakdown for GSTR-1 filing
										</Card.Description>
									</div>
									...
									<Table.Body>
										{#each summary.hsnSummary as hsn}
											<Table.Row>
												...
												<Table.Cell class="text-right tabular-nums">{formatNumber(hsn.quantity)}</Table.Cell>
												<Table.Cell class="text-right">
													<Badge variant="outline" class="text-xs">{hsn.rate}%</Badge>
												</Table.Cell>
												<Table.Cell class="text-right tabular-nums">{formatCurrency(hsn.taxableValue)}</Table.Cell>
												<Table.Cell class="text-right tabular-nums text-blue-600">{formatCurrency(hsn.cgst)}</Table.Cell>
												<Table.Cell class="text-right tabular-nums text-purple-600">{formatCurrency(hsn.sgst)}</Table.Cell>
												{#if summary.igstCollected > 0}
													<Table.Cell class="text-right tabular-nums text-orange-600">{formatCurrency(hsn.igst)}</Table.Cell>
												{/if}
												<Table.Cell class="text-right tabular-nums font-semibold text-amber-600">{formatCurrency(hsn.totalTax)}</Table.Cell>
											</Table.Row>
										{/each}
									</Table.Body>
								</Table.Root>
