<script lang="ts">
	...
	interface Props {
		/** If set, the logo renders as a link to this href (default '/'). Pass `null` for a non-interactive mark. */
		href?: string | null;
		/** Mark + wordmark scale. */
		size?: Size;
		/** Show the `{APP_NAME}` wordmark next to the mark. */
		showWordmark?: boolean;
		class?: string;
	}

	let { href = '/', size = 'md', showWordmark = true, class: className = '' }: Props = $props();

	const box: Record<Size, string> = { sm: 'size-7', md: 'size-9', lg: 'size-11' };
	const svg: Record<Size, number> = { sm: 15, md: 18, lg: 22 };
	const word: Record<Size, string> = { sm: 'text-base', md: 'text-lg', lg: 'text-xl' };

	const interactive = $derived(href !== null);
</script>

<!--
	Shared brand mark for public / auth / ordering surfaces — the single source of the
	logo so those flows stop drifting into three unrelated skins (see UI-consistency audit).
	The grid mark + APP_NAME wordmark mirrors the public site header (the Tier-2 north star).
-->
{#snippet mark()}
	<span
		class="flex {box[
			size
		]} shrink-0 items-center justify-center rounded-lg bg-primary text-primary-foreground {interactive
	...
	{#if showWordmark}
		<span class="{word[size]} font-semibold tracking-tight">{APP_NAME}</span>
	{/if}
{/snippet}
...
{#if interactive}
	<a
		...
		class="group flex items-center gap-2.5 {className}"
	>
		{@render mark()}
	</a>
{:else}
