...
<div class={cn(emptyStateVariants({ size }), className)} data-slot="empty-state">
	<!-- Illustration/Icon -->
	<div
		class={cn('flex items-center justify-center rounded-full', bgColorClass, iconContainerClass)}
	>
	...
	{#if actionLabel || secondaryActionLabel}
		<div class="flex flex-wrap items-center justify-center gap-2">
			...
			{#if secondaryActionLabel}
				<Button variant="ghost" onclick={onSecondaryAction} size={size === 'sm' ? 'sm' : 'default'}>
					{secondaryActionLabel}
				</Button>
			{/if}
		</div>
