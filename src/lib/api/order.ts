...
export interface OrderItem {
	...
	cancellationReason?:
		'customer_changed_mind' | 'out_of_stock' | 'wrong_item' | 'kitchen_error' | 'other';
...
export async function updateItemQuantity(
	...
	const api = options?.fetch ? createApiClient({ fetch: options.fetch }) : getApiClient();
	return api.patch(`/business/${businessId}/orders/${orderId}/items/quantity`, {
		itemId,
		quantity
	});
}
...
export async function bulkUpdateItemStatuses(
	...
	const api = options?.fetch ? createApiClient({ fetch: options.fetch }) : getApiClient();
	return api.patch(`/business/${businessId}/orders/${orderId}/items/bulk-status`, {
		itemIds,
		status
	});
}
