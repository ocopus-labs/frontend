 ...
 */
export async function getSubscriptionUsage(options?: FetchOption): Promise<{ usage: UsageStats }> {
	const api = options?.fetch ? createApiClient({ fetch: options.fetch }) : getApiClient();
 ...
 */
export async function getCustomerPortalUrl(options?: FetchOption): Promise<PortalResponse> {
	const api = options?.fetch ? createApiClient({ fetch: options.fetch }) : getApiClient();
...
export function getPlanFeaturesList(plan: SubscriptionPlan): string[] {
	...
	// Limits
	features.push(
		`${formatLimit(plan.maxLocations)} business location${plan.maxLocations !== 1 ? 's' : ''}`
	);
	features.push(
		`${formatLimit(plan.maxTeamMembers)} team member${plan.maxTeamMembers !== 1 ? 's' : ''}`
	);
	features.push(
