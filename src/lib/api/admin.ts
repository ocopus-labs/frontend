...
export async function getAdminLiveStats(options?: { fetch?: typeof fetch }): Promise<LiveStats> {
	const api = options?.fetch ? createApiClient({ fetch: options.fetch }) : getApiClient();
...
export async function bulkUserAction(data: {
	ids: string[];
	action: string;
	reason?: string;
...
export async function bulkBusinessAction(data: {
	ids: string[];
	action: string;
}): Promise<{ processed: number; failed: number }> {
	const api = getApiClient();
...
export async function bulkSubscriptionAction(data: {
	ids: string[];
	action: string;
}): Promise<{ processed: number; failed: number }> {
	const api = getApiClient();
...
export async function getAdminPlans(options?: {
	fetch?: typeof fetch;
...
export async function exportAdminBusinesses(params?: {
	status?: string;
	type?: string;
	search?: string;
}): Promise<Blob> {
...
export async function exportAdminSubscriptions(params?: {
	status?: string;
	planId?: string;
}): Promise<Blob> {
	const searchParams = new URLSearchParams();
...
export async function getAdminSystemHealth(options?: { fetch?: typeof fetch }): Promise<{
	status: string;
	uptime: number;
