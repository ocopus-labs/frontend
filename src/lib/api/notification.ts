...
export async function removeDeviceToken(businessId: string, token: string, options?: FetchOption) {
	const api = options?.fetch ? createApiClient({ fetch: options.fetch }) : getApiClient();
	return api.delete(
		`/business/${businessId}/notifications/device-token/${encodeURIComponent(token)}`
	);
}
