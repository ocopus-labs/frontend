...
export async function getOrderStats(
	...
	const response = await api.get<{ stats: OrderStats }>(
		`/business/${businessId}/orders/stats${query}`
	);
...
export async function getPaymentSummary(
	...
	const api = options?.fetch ? createApiClient({ fetch: options.fetch }) : getApiClient();
	const query = date ? `?date=${date}` : '';
	const response = await api.get<{ summary: PaymentSummary }>(
		`/business/${businessId}/payments/summary${query}`
	);
